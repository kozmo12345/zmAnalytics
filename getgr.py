...
for mesui, mstime in enumerate(mstimes):
    dataFile = open(os.path.join("C:\\", "Data\\grdata" + str(mesui) + ".txt"), 'w')
    dataFile.write( 'date,grade,code,sgrad,rgrad,rsd,gr,maxgr,mesu,maxc_msc,cost\n')
            ...
            for ci, code in enumerate(codes):
                ...
                for i, b_currentTime in enumerate(exportData[:,0]):
                    ...
                    if(b_currentTime.decode('utf-8') == str_standardTime and grade < 15 and ms != 0 and md != 0):
                        ...
                        maxgr[code.decode('utf-8')] = (max(exportData[:,4].astype(float)))
