...
otimes = sp.unique(data[data[:,0] != b''][:,0])
...
for ottime in otimes[:]:
    termData = termData[termData[:,0] != ottime]
    oTimeindex = oTimeindex + 1

    data = termData;
    codes = sp.unique(data[data[:,7] != b''][:,7])
    times = sp.unique(data[data[:,0] != b''][:,0])
    if(time.strptime(ottime.decode('utf-8'), '%H:%M:%S').tm_min > 12):
        break;
    tmp_time = 0
    mesuDict = dict()
    mesuStart = dict()
        ...
        try:
            ...
            if(bool_oTime == True):
                ...
                for code in (codes):
                    ...
                    if(second_oTime > endTime or code in completedCodes):
                        continue;
                    ...
                    if(((ms_md > 0.96 and sms_md > 1 and gr > 420000 ) or (cggrad > 2.3 and chegang > 163 > gradient > 1.5)) and grade < 20 and exportData[i, 3].astype(float) > 5):
                        x = ti
                        ...
                        if(gradient >= 0.7 and srgrad > 0):
                            ...
                            if(mesuDict[code.decode('utf-8')] in mesuLimit and ((code) not in comps) and (code not in nos)):
                                ...
                                if(oTimeindex > 1 and exportData[i, 3].astype(float) < 11):
                                    nos.append(code)
                                    continue;
