                    ...
                    if(bool_oTime == True):
                        ...
                        for code in (codes):
                            ...
                            if(code in comps and code.decode('utf-8') in mesuStart and mesuDict[code.decode('utf-8')] >= 3):
                                mmRate  = (sp.sum(exportData[i-(lange*4-2):i+1,5].astype(float)))/(sp.sum(exportData[i-(lange*4-2):i+1,6].astype(float)))
                                if(today.split('-')[1] == '09' or (today.split('-')[1] == '10' and today.split('-')[2] == '04')):
                                    mmRate = (sp.sum(exportData[i-lange:i+1,5].astype(float)))/(sp.sum(exportData[i-lange:i+1,6].astype(float)))
