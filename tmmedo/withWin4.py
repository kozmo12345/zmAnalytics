...
with open(setFilePath, 'r') as f:
    for line in f:
        ...
        if(imCode != ''):
            isRe = True
        if(imCode != '' and int(line.split(",")[8].strip()) == 4):
            ...
            pick[imCode] = False
            endIndex[imCode] = 0
        elif(imCode != '' and int(line.split(",")[8].strip()) != 4):
...
while(True):
    ...
    try:
        ...
        wchkfilePath = os.path.join("C:\\", "Dropbox\\Data\\" + today + "\\" + "w4.pchk");
        ...
        for ttime in (times):
            ...
            for code in (codes):
                ...
                if(code.decode('utf-8') in delayMesu and delayMesu[code.decode('utf-8')] + 1 < i and delayMesu[code.decode('utf-8')] + 6 > i and grRate < 0.89 and (int(exportData[i, 4].decode('UTF-8')) - int(exportData[i-1, 4].decode('UTF-8'))) > 1000 and (int(exportData[i, 4].decode('UTF-8')) - int(exportData[i-1, 4].decode('UTF-8'))) > 1000 and code not in comps and chegang < 400):
                    ...
                    setFile.write( str(code.decode('utf-8')) + ',' + str(float(rate)) + ',' + str(tpg) +  ',' + str_oTime + ',' + str(wanna) + ',' + str(datetime.datetime.now().strftime('%H:%M:%S')) + ',' + str(cost) + ',' + str(second_oTime) + ',' + str(4) + '\n')
                    setFile.close()
                    del delayMesu[code.decode('utf-8')]
