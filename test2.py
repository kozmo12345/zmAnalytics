                    ...
                    for code in (codes):
                        ...
                        if(code in comps):
                            ...
                            if(pick[code.decode('utf-8')] and exportData[i, 3].astype('float') < 19):
                                edFile.write( str(code.decode('utf-8')) + ',' + str(allMax) +  ',' + str(termMin) + ',' + str(termMax) + ',' + str(md) + ',' + str(ms) + ',' + str(red) + ',' + str(msTime) + ',' + str(mdTime) + ',' + str(msCost) + ',' + str(mdCost) + ',' + str(msGradient[code.decode('utf-8')]) + ',' + str(msGr[code.decode('utf-8')]) + ',' + str(msSmdms[code.decode('utf-8')]) + ',' + str(msGrade[code.decode('utf-8')]) + ',' + str(msSrgrad[code.decode('utf-8')]) + ',' + str(nosDic[code.decode('utf-8')]) + '\n')
                                comps.remove(code)
                                sumEd = sumEd + red
                                medos.append(code)
                        ...
                        if(((ms_md > 0.96 and sms_md > 1 and gr > 420000) or (cggrad > 2.3 and chegang > 163)) and grade < 16 and exportData[i, 3].astype(float) > 5):
                            ...
                            if(gradient >= (0.73 + 0.03 * wlevel) and srgrad > 0 and ltdc > 0):
                                ...
                                if(mesuDict[code.decode('utf-8')] in mesuLimit and ((code) not in comps) and (code not in medos) and (code not in nos)):
                                    ...
                                    if((ms_md > 0.96 and sms_md > 1 and gr > 420000)):
                                        nosDic[code.decode('utf-8')].append('16')

                                    if(cggrad > 2.3 and chegang > 163):
                                        nosDic[code.decode('utf-8')].append('17')
                                    ...
                                    if(cost > 7500):
                                        nosDic[code.decode('utf-8')].append('4')
                                    ...
                                    if((chegang < 129 and lcggrad < -1.8)):
                                        nosDic[code.decode('utf-8')].append('11')

                                    if(lcggrad > 60):
                                        nosDic[code.decode('utf-8')].append('12')

                                    if(fcggrad < -12.8):
                                        nosDic[code.decode('utf-8')].append('13')
                                    
                                    if(len(x) > 5):
                                        fit = sp.polyfit(x[:6], exportData[i-5:i+1,3].astype(float), 1)
                                        gradif = sp.around(fit[0]*10, decimals=2)
                                    else:
                                        gradif = 0
                                    if(fcggrad < -10 and gradif > 1.8):
                                        nosDic[code.decode('utf-8')].append('15')              
                                    ...
                                    if(tgrad < -6):
                                        nosDic[code.decode('utf-8')].append('14')
                                    ...
                                    msSrgrad[code.decode('utf-8')] = str(cost)
