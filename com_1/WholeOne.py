                ...
                if(bool_oTime == True):
                    ...
                    for code in (codes):
                        ...
                        if(code in comps):
                            ...
                            nosDic[code.decode('utf-8')] = exportData[mesuStart[code.decode('utf-8')]+6,3].astype(float) - exportData[mesuStart[code.decode('utf-8')],3].astype(float)
                            ...
                            elif((i - mesuStart[code.decode('utf-8')]) % 6 == 0 and ed > 1.92):
                                edFile.write( str(code.decode('utf-8')) + ',' + str(allMax) +  ',' + str(termMin) + ',' + str(termMax) + ',' + str(md) + ',' + str(ms) + ',' + str(red) + ',' + str(msTime) + ',' + str(mdTime) + ',' + str(msCost) + ',' + str(mdCost) + ',' + str(msGradient[code.decode('utf-8')]) + ',' + str(msGr[code.decode('utf-8')]) + ',' + str(msSmdms[code.decode('utf-8')]) + ',' + str(msGrade[code.decode('utf-8')]) + ',' + str(msSrgrad[code.decode('utf-8')]) + ',' + str(nosDic[code.decode('utf-8')]) +  ',' + str(mesuIndex[code.decode('utf-8')]) + '\n')
                                comps.remove(code)
                                sumEd = sumEd + red
                                medos.append(code)
                        ...
                        if(code.decode('utf-8') in delayMesu and delayMesu[code.decode('utf-8')] + 1 < i and delayMesu[code.decode('utf-8')] + 6 > i and grRate < 0.89 and (int(exportData[i, 4].decode('UTF-8')) - int(exportData[i-1, 4].decode('UTF-8'))) > 1000 and code not in comps and chegang < 400):
                            ...
                            msGrade[code.decode('utf-8')] = (exportData[i+26,4].astype(float) - exportData[i+24,4].astype(float)) * exportData[i+25,8].astype(float)
                            msSrgrad[code.decode('utf-8')] = float(exportData[i+26, 3].decode('UTF-8')) - float(exportData[i, 3].decode('UTF-8'))
                            ...
                            mesuIndex[code.decode('utf-8')] =float(exportData[i+16, 3].decode('UTF-8')) -float(exportData[i, 3].decode('UTF-8'))  
                        ...
                        if(((ms_md > 0.96 and sms_md > 1 and gr > 330000 and not (cggrad < -4 and chegang < 160)) or (cggrad > 2 and chegang > 159)) and grade < 16 and exportData[i, 3].astype(float) > 5 and code.decode('utf-8') not in delayMesu and gr1):
                            x = sp.array(range(i+1))
                ...
                if(bool_oTime == True):
                    ...
                    for code in (codes):
                        ...
                        if(code.decode('utf-8') in delayMesu and delayMesu[code.decode('utf-8')] + 1 < i and delayMesu[code.decode('utf-8')] + 6 > i and grRate < 0.89 and (int(exportData[i, 4].decode('UTF-8')) - int(exportData[i-1, 4].decode('UTF-8'))) > 1000 and code not in comps and chegang < 400):
                            ...
                            ones.append([code.decode('utf-8'), today, second_oTime, 3])
                            comps.append((code))
                            mesuStart[code.decode('utf-8')] = i
                            msGradient[code.decode('utf-8')] = gradient
                            msGr[code.decode('utf-8')] = gr
                        ...
                        if(code.decode('utf-8') not in cggradDic):
                            ...
                            cggradDic[code.decode('utf-8')].append(cggrad)                        
                        cost = int(exportData[i, 8].decode('UTF-8'))    
                        if(((cggrad > 2 and chegang > 159)) and grade < 16 and exportData[i, 3].astype(float) > 5 and code.decode('utf-8') not in delayMesu and gr1):
                            x = sp.array(range(i+1))
                            y = exportData[:i+1,3].astype(float)
                            if(len(y) <= 1):
                                break
...
for dirname, dirnames, filenames in os.walk("C:\\Dropbox\\com_1\\diff\\"):
    for subdirname in dirnames:
        ...
        print(today)
        setFile = open(os.path.join("C:\\", "Dropbox\\com_1\\diff\\" + today + "\\" + today + "moa3.txt"), 'w')
        edFile = open(os.path.join("C:\\", "Dropbox\\com_1\\diff\\" + today + "\\" + today + "ed2.txt"), 'w')
        realfilePath = os.path.join("C:\\", "Dropbox\\com_1\\diff\\" + today + "\\" + today + ".txt");
                ...
                if(bool_oTime == True):
                    ...
                    for code in (codes):
                        ...
                        if(code.decode('utf-8') in delayMesu and delayMesu[code.decode('utf-8')] + 1 < i and delayMesu[code.decode('utf-8')] + 6 > i and grRate < 0.89 and (int(exportData[i, 4].decode('UTF-8')) - int(exportData[i-1, 4].decode('UTF-8'))) > 1000 and (int(exportData[i, 4].decode('UTF-8')) - int(exportData[i-1, 4].decode('UTF-8'))) > 1000 and code not in comps and chegang < 400):
                            ...
                            ones.append([code.decode('utf-8'), today, second_oTime, 4])
                            comps.append((code))
                            mesuStart[code.decode('utf-8')] = i
                            msGradient[code.decode('utf-8')] = gradient
                            msGr[code.decode('utf-8')] = gr
...
for code, today, time, no in ones:
    ...
    if(len([x for x in twos if (x[0] == code and x[1] == today)]) > 0):
        continue;
    
    matchs.sort(key=lambda x: x[2])

    twos.append(matchs[0])
...
for dirname, dirnames, filenames in os.walk("C:\\Dropbox\\com_1\\Data\\"):
    for subdirname in dirnames:
        ...
        for line in setFile:
            item = [x for x in twos if (x[0] == line.split(',')[0] and x[1] == today and x[3] == 1)]
            if(len(item) == 0):
                continue;
...
for dirname, dirnames, filenames in os.walk("C:\\Dropbox\\com_1\\diff\\"):
    for subdirname in dirnames:
        ...
        for line in setFile:
            item = [x for x in twos if (x[0] == line.split(',')[0] and x[1] == today and x[3] == 2)]
            if(len(item) == 0):
                continue;            
            analFile.write(today + ',' + line)
...
for dirname, dirnames, filenames in os.walk("C:\\Dropbox\\com_1\\Data\\"):
    for subdirname in dirnames:
        ...
        for line in setFile:
            item = [x for x in twos if (x[0] == line.split(',')[0] and x[1] == today and x[3] == 3)]
            if(len(item) == 0):
                continue;
            analFile.write(today + ',' + line)
...
for dirname, dirnames, filenames in os.walk("C:\\Dropbox\\com_1\\diff\\"):
    for subdirname in dirnames:
        ...
        for line in setFile:
            item = [x for x in twos if (x[0] == line.split(',')[0] and x[1] == today and x[3] == 4)]
            if(len(item) == 0):
                continue;            
            analFile.write(today + ',' + line)

        set2FilePath = os.path.join("C:\\", "Dropbox\\com_1\\diff\\" + today + "\\" + today + "ed2.txt");
        set2File = open(set2FilePath, 'r')
