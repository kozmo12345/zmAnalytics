...
startTime = datetime.timedelta(hours=14,minutes=50,seconds=00).total_seconds()
endTime = datetime.timedelta(hours=15,minutes=2,seconds=30).total_seconds()
fMedoTime = datetime.timedelta(hours=15,minutes=18,seconds=50).total_seconds()
...
while(True):
    ...
    try:
        ...
        for ttime in (times):
            ...
            ttimeData5 = ttimeData4[ttimeData4[:,8].astype(float) > 1000]
            codes = ttimeData5[:,7]
            ...
            for code in (codes):
                ...
                if(code in comps and code not in medos and code.decode('utf-8') in mesuStart and second_oTime > mesuStart[code.decode('utf-8')] + 10):
                    ...
                    if(nowTime - 20 < second_oTime):
                        ...
                        if(mesuStart[code.decode('utf-8')] + 510 < second_oTime):
                ...
                if(((ms_md > 0.96 and sms_md > 1 and gr > 330000 and not (cggrad < -4 and chegang < 160)) or (cggrad > 2.3 and chegang > 163)) and grade < 16 and exportData[i, 3].astype(float) > 5 and gr1 and cost > 2240 and cost < 9000):
