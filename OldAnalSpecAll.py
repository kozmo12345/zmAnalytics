                        ...
                        if(ms_md > 1 and sms_md > 1):
                            ...
                            if(gradient >= 0.7 and srgrad > -0.01):
                                ...
                                if(mesuDict[code.decode('utf-8')] == 3):
                                    setFile.write( str(code.decode('utf-8')) +  ',' + str(float(exportData[i+1, 3].decode('UTF-8'))) + ',' + str(exportData[maxc + i + 1,3].decode('UTF-8')) + ',' + str_oTime + ',' + str(gr)  + ',' + str(i)  + ',' + str( min(exportData[i:i + maxc + 1, 3].astype(float)) )  + ',' + str( max(exportData[:i, 3].astype(float)) )  + ',' + str(exportData[i +maxc, 0].decode('UTF-8')) +  ',' + str( grade )  +  ',' + str( gradient )  +  ',' + str( srgrad )  +  ',' + str( sms_md )  +  '\n')
...
analFile.write( 'day,code,nextRate,maxRate,mesuTime,gr,index,minRate,nowMaxRate,maxTime,grade,gradient,srgrad,sms_md\n')
