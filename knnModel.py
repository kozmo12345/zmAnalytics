...
analFile = open(analFilePath, 'r')

features = []
labels = []

for line in analFile:
    features.append(line.split(',')[:-3])
    labels.append(line.split(',')[-1])

features = sp.asarray(features).astype(float)
labels = sp.asarray(labels)
...
for training,testing in kf:
    ...
    # 이 중첩에 대해 정확도를 반환하여 불 배열에 대해 sp.mean 사용
...
labels = sp.array([names.index(ell) for ell in labels])
preds = labels.copy()
preds[:] = -1
